...
for (let i = 0 ; i < 3; i++ ){
    console.log(`the Bill was ${bill[i]}, the tip was ${tipcalculator(bill[i])} , and the total value ${bill[i] + tipcalculator(bill[i])}`)
}


// OUtra opção de LOOP (forEach)
bill.forEach(value => { 
    console.log(`the Bill was ${value}, the tip was ${tipcalculator(value)} , and the total value ${value + tipcalculator(value)}`)
} ) 
